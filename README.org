...
* COMMENT Notes
** Drops down to NumPy for numeric calculations, obviating any Pandas issues
#+BEGIN_SRC python
...
1. Long to wide
2. APIs/web/requests
3. Exceptions/testing
4. Data cleanup
5. Joins
6. Database access
7. Quick graphs with seaborn plus fiddling with matplotlib
8. Tour of statistical libraries
9. Formatted strings for reports
...
2. Basic data recoding/labeling
...
5. Basic Inferential Statistics
   ...
   3. T Test
   4. Anova
6. Advanced Stats (what libraries to use; where to find documentation)
...
*** COMMENT *(Optional) Beginner Challenge*
...
*** COMMENT Motivating example: Calculate the wealth Z-score for each country
Revise this to just use the mean, skip the Z scoring
#+BEGIN_SRC python
...
*** COMMENT *Hard Challenge*: The perils of missing data
...
*** COMMENT (Optional) Importing data from databases
#+BEGIN_SRC python
...
** COMMENT (Optional) Pandas method chaining in the wild
...
** COMMENT (Carpentries version) Group By: split-apply-combine
...
*** COMMENT Deprecated Challenge small files
#+BEGIN_SRC python
for filename in glob.glob('data/*.csv'):
    data = pd.read_csv(filename)
    if len(data) < 30:
...
*** COMMENT *Challenge:* Find the European data
...
** COMMENT (Carpentries version) Conditionals
*** Use ~if~ statements to control whether or not a block of code is executed
