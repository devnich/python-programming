...
*** Pandas help files are dense; prefer the online documentation
...
#+BEGIN_SRC python
# Create a new list that only contains methods or attributes
# NB: Because Python allows you to override any method or attribute at runtime,
# testing with `callable` may not always work correctly.
...
- Each Series (column) is a 1-dimensional NumPy array
- Therefore, each series inherits the abilities (linear algebra) and limitations (single data type) of NumPy
...
For more elaborate comparisons, see https://pandas.pydata.org/docs/user_guide/basics.html#basics-compare
...
#+BEGIN_SRC python
#+END_SRC
Curriculum: https://scipy-lectures.org/packages/statistics/
