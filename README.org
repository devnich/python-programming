...
2. Arrays are fast but inflexible - the entire array must be of a single type.
...
2. Use ~:~ on its own to mean all columns or all rows. This is Python’s usual slicing notation, which allows you to treat DataFrames as multi-dimensional lists. Note that Pandas automatically converts the 1-dimensional row into a Series; we will discuss how to get around this in the next section.
   #+BEGIN_SRC python
...
6. (Optional) You can extract individual rows as Series, then concatenate them into a new DataFrame
   #+BEGIN_SRC python
...
2. ~.filter()~ always returns the same type as the original item, whereas ~.loc~ and ~.iloc~ might return a data frame or a series.
   #+BEGIN_SRC python
...
2. Join tables on common column. The inner join only includes rows where both tables match on the key column.
   #+BEGIN_SRC python
...
3. Get the subset of species that match a criterion, and join on that subset.
   #+BEGIN_SRC python
